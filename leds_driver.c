...
#define SUCCESS 0
#define DEVICE_NAME "leds_driver"	/* Dev name as it appears in /proc/devices   */
...
device_write(struct file *filp, const char *buff, size_t len, loff_t *off){
    ...
    int i;
    char* kmsg = (char*)vmalloc(len);
    unsigned int leds = 0;
    if (NULL == kmsg) {
        return -ENOMEM;
    }
    if (copy_from_user(kmsg, buff, len))
        return -EFAULT;
    //kmsg[strlen(kmsg)-1] = '\0';
    //printk(KERN_INFO "WRITE OP: %s\n", buff);
    printk(KERN_INFO "WRITE OP: %s\n", kmsg);
    printk(KERN_INFO "strlen  : %zu\n", strlen(kmsg));
    printk(KERN_INFO "len     : %zu\n", len);

    for(i=0; i<strlen(kmsg); i++){
        if(kmsg[i] == '1')
            leds |= LED_NUM;
        else if(kmsg[i] == '2')
            leds |= LED_CAP;
